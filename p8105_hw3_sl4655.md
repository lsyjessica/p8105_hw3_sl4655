...
acc_data <- read_csv(file = "./Data/accel_data.csv") %>%
  ...
  select(week, day, hour, minute, activity_per_minute)
```
...
This dataset has 50400 observations of 5 variables:week, day, hour, minute, activity\_per\_minute.
...
acc_data %>%
  group_by(week, day) %>% 
  summarise(total_activity = sum(activity_per_minute))
```
